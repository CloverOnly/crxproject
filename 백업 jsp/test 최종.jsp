	...
	<script>	
		...
		function formatDate() {
		   ...
		   function fetchTicketInfo() {
		       ...
		       var seatType = document.querySelector('select').value;
		                       ...
		                       if (items.length > 0) {
		                           ...
		                           items.forEach(function(item) {
		                               ...
		                               table += '<tr>' +
		                                   ...
		                                   '<td><button class="seat-btn" onclick="seatTicket(\'' + seatType + '\')">좌석 선택</button></td>' +
		                                   '<td>' + formattedTime + '</td>' +
		...
		function seatTicket() {
		    window.name = "parentForm";
		    openWin3 = window.open("seat.do", "seat", "width=800, height=350, resizable=no, scrollbars=no");
		}
		function seatTicketValue() {
		    if (seat && !seat.closed) {
		        seat.document.getElementById("seatInput").value = document.getElementById("seatname").value;
		    } else {
		        alert("창이 닫혔거나 열리지 않았습니다.");
		    }
		}
