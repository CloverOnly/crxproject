...
<head>
	...
	<script>
	    ...
	    function toggleSeat(sectionId, buttonElement) {
	        ...
	        selectedHocha = buttonElement.value; // value 값만 저장
	    ...
	    function abcdSeat(buttonElement) {
	        ...
	        if (!isSelected && selectedSeats.length < totalCount) {
	            buttonElement.classList.add('selected');
	            selectedSeats.push(buttonElement.value); // value 값만 저장
	        } else if (isSelected) {
	            buttonElement.classList.remove('selected');
	            selectedSeats = selectedSeats.filter(seat => seat !== buttonElement.value);
	        }
	        
	        updateSelectionInfo(); // 좌석 정보와 호차 정보 업데이트

	        console.log('Selected Seats:', selectedSeats); // 추가된 로그
	        console.log('Selected Hocha:', selectedHocha);  // 추가된 로그
	    }
	    ...
	    function seatForm() {
	        ...
	        console.log('Selected Hocha:', selectedHocha);
...
<body>
    ...
    <div class="seatAll">
		...
		<form id="seatForm" action="${pageContext.request.contextPath}/seat.do" method="post">
		    ...
		    <!-- 버튼 클릭 시 폼을 제출합니다 -->
		    <input type="submit" id="seatReservation" value="좌석 예매하기" onclick="seatForm(); return false;">
		</form>
