...
<head>
    ...
    <script>
		...
		function toggleSeat(sectionId, buttonElement) {
		    ...
		    // 좌석 상태 업데이트
		    updateDisabledSeats(selectedHocha);
		    resetSeatSelection();
		    updateSelectionInfo();
		}
		
		// 선택된 호차의 비활성화된 좌석 업데이트
		function updateDisabledSeats(hocha) {
		    disabledSeats = [];

		    rSeats.forEach(seatInfo => {
		        if (seatInfo.hocha === hocha) {
		            // 좌석 비활성화 데이터 처리
		            seatInfo.selectedSeats.split(',').forEach(seat => {
		                disabledSeats.push(seat);
		            });
		        }
		    });
...
<body>
    ...
    <div class="seatAll">
        ...
        <br>
        <div class="inf">
			<span><img src="../img/seat2.jpg" alt="선택 좌석" class="selectseat" /></span><span>선택좌석</span>  
			<span><img src="../img/seat3.jpg" alt="선택 좌석" class="selectseat" /></span><span>선택불가</span> 
			<span><img src="../img/seat1.jpg" alt="선택 좌석" class="selectseat" /></span><span>선택가능</span>
        </div>
        <br>
		<ul class="line"></ul>
