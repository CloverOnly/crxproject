...
/*!50001 VIEW `v_train_seat` AS select `reservation`.`selectedSeats` AS `selectedSeats`,`reservation`.`hocha` AS `hocha`,`reservation`.`startdate` AS `startdate`,`reservation`.`trainType` AS `trainType`,`reservation`.`trainno` AS `trainno`,`reservation`.`depTime` AS `depTime`,`reservation`.`arrTime` AS `arrTime` from `reservation` union select `paytrain`.`selectedSeats` AS `selectedSeats`,`paytrain`.`hocha` AS `hocha`,`paytrain`.`startdate` AS `startdate`,`paytrain`.`trainType` AS `trainType`,`paytrain`.`trainno` AS `trainno`,`paytrain`.`depTime` AS `depTime`,`paytrain`.`arrTime` AS `arrTime` from `paytrain` */;
...
-- Dump completed on 2024-08-29 17:31:46
